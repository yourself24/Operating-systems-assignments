...
void semErrCheck(int semid, char *msg)
{
    if (semid < 0)
    {
        printf("Error creating the semaphore for %s", msg);
        exit(1);
    }
}
...
int main(int argc, char **argv)
{
    ...
    semErrCheck(semP2, "P2");
    semctl(semP2, 0, SETVAL, 0); // for sync of T6.2 and T2.2
    semctl(semP2, 1, SETVAL, 0); // for sync of T6.2 and T2.1
    pid[0] = fork();
    if (pid[0] == 0)
    {
        info(BEGIN, 2, 0);
    ...
    if (pid[1] == 0)
    {
        ...
        if (pid[2] == 0)
        {
            ...
            if (pid[4] == 0)
            {
                ...
                semctl(sem64, 0, SETVAL, 0); // makes sure T6.4 starts after T6.5
                semctl(sem64, 1, SETVAL, 0); // makes sure T6.4 ends after T6.5
                semErrCheck(sem64, "P6");
                info(BEGIN, 6, 0);
                for (int i = 1; i < 6; i++)
                {
